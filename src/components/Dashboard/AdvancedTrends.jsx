    ...
    <Card>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-3">
          ...
          <span className="flex items-center gap-1 text-[10px] text-text-muted"><span className="w-2 h-2 rounded-full bg-[#6366F1]" />Advance</span>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={data}>
          ...
          <Area type="monotone" dataKey="Advance" stroke="#6366F1" strokeWidth={2} fill="url(#gradAdv)" dot={false} />
        </AreaChart>
      </ResponsiveContainer>
    </Card>
  );
    ...
    <Card>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-3">
          ...
          <span className="flex items-center gap-1 text-[10px] text-text-muted"><span className="w-2 h-2 rounded-full bg-[#F43F5E]" />Customize</span>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={data}>
          ...
          <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 9, fill: '#F43F5E' }} domain={[0, 100]} tickFormatter={(v) => `${v}%`} axisLine={false} tickLine={false} width={35} />
          <Tooltip cursor={{ stroke: 'none' }} content={<TrendTooltip />} />
          <Area yAxisId="left" type="monotone" dataKey="Regular" stroke="#C9A84C" strokeWidth={2} fill="url(#gradRegular)" />
          <Area yAxisId="left" type="monotone" dataKey="Customize" stroke="#F43F5E" strokeWidth={2} fill="url(#gradCustom)" />
          <Line yAxisId="right" type="monotone" dataKey="Custom %" stroke="#F43F5E" strokeWidth={1.5} strokeDasharray="4 3" dot={false} />
        </ComposedChart>
      </ResponsiveContainer>
    </Card>
  );
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-3">
          <span className="flex items-center gap-1 text-[10px] text-text-muted"><span className="w-2 h-2 rounded-full bg-[#6366F1]" />AOV</span>
      ...
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={data}>
          ...
          <Tooltip cursor={{ stroke: 'none' }} content={<TrendTooltip />} />
          <Bar yAxisId="left" dataKey="AOV" fill="#6366F1" fillOpacity={0.7} radius={[3, 3, 0, 0]} barSize={12} />
